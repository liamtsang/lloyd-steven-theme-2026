...
class MenuDrawer extends HTMLElement {
  constructor() {
    ...
    // Add mouse leave event listener to close when mouse leaves the menu
  ...
  onKeyUp(event) {
    ...
    const openDetailsElement = event.target.closest('details[open]');
    if (!openDetailsElement) return;2
  ...
  closeMenuDrawer(event, elementToFocus = false) {
    ...
    // Let the animation handle closing the main details element
...
loadContent(focus = false) {
  ...
  if (!this.getAttribute('loaded')) {
    ...
    deferredElement.style.display = 'block'; // Ensure the video is displayed
    if (focus) deferredElement.focus();

    if (deferredElement.nodeName === 'VIDEO' && deferredElement.getAttribute('autoplay')) {
      deferredElement.muted = true; // Mute for autoplay to work in most browsers
...
class SliderComponent extends HTMLElement {
  ...
  update() {
    ...
    if (this.currentPageElement && this.pageTotalElement) {
      ...
      // Subtrtacting 1 page not sure why I needed to do this it is a hack
...
class VariantSelects extends HTMLElement {
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    // Custom
    const submit_cta = document.getElementById('ProductSubmitButtonCTA')
    const submit_divider = document.getElementById('ProductSubmitButtonDivider')
    const submit_details = document.getElementById('ProductSubmitButtonDetails')

    if (this.currentVariant && this.inventoryData[this.currentVariant.id] > 0 && !disable) {
      submit_cta.textContent = 'Add to Cart'
      submit_divider.textContent = '|'
      submit_details.textContent = 'In Stock'
    } else if (this.currentVariant && this.inventoryData[this.currentVariant.id] == 0 && !disable) {
      submit_cta.textContent = 'Add to Cart'
      submit_divider.textContent = '|'
      submit_details.textContent = 'Made to Order'
    }
    // End
    ...
    if (disable) {
      ...
      submit_cta.textContent = text
      submit_divider.textContent = ''
      submit_details.textContent = ''
    } else {
  ...
  getVariantData() {
    const variantJSON = JSON.parse(this.querySelector('[type="application/json"]').textContent);
    this.variantData = variantJSON.variants;
    this.inventoryData = variantJSON.inventories;
